...
CREATE VIEW [dbo].[viewEmployeeDetails] AS SELECT e.EmpCode Code,CONCAT(e.EmpLNEN,' ',e.EmpFNEN) EmployeeNameEN,CONCAT(e.EmpLNKH,' ',e.EmpFNKH) EmployeeNameKH,e.Gender,e.IDCard NationalID,e.Address,e.Phone,e.Email,p.PosName PositionName,e.Salary,e.JoinDate
FROM Employee e
INNER JOIN Position p
ON e.PosID = p.PosID;
GO
...
CREATE VIEW [dbo].[viewPurchase] AS SELECT p.PurCode PurCode,p.Date PurDate,pp.PName ProName,pd.Quantity Quantity,pd.Unitprice,pd.Amount Amount, p.Payment Payment,p.Balance Balance,CONCAT(s.SuLNEN,' ',s.SuFNEN) SupplierName,CONCAT(e.EmpLNEN,' ',e.EmpFNEN) EmployeeName,p.Total
